...
export default function AdminDashboard() {
  ...
  const [viewProperty, setViewProperty] = React.useState(null);
        ...
        <section className="admin-main">
          ...
          {section === "properties" ? (
            <EntityTable
              ...
              renderRight={(row) => (
                <button className="btn btn-ghost" onClick={() => window.open(`/#properties`, '_blank')}>View Site</button>
              )}
...
function Overview({ stats }) {
  return (
    <div>
      ...
      <div className="stats">
        <Stat title="Properties" value={stats?.properties ?? "0"} icon="fa-home" />
        <Stat title="Maps" value={stats?.maps ?? "0"} icon="fa-map" />
        <Stat title="Certificates" value={stats?.certificates ?? "0"} icon="fa-award" />
        <Stat title="Inquiries" value={stats?.inquiries ?? "0"} icon="fa-comments" />
        <Stat title="Subscribers" value={stats?.subscribers ?? "0"} icon="fa-users" />
      </div>
      <div className="hint mt-16" style={{ background: 'rgba(13, 59, 102, 0.05)', color: 'var(--primary)', border: '1px solid rgba(13, 59, 102, 0.1)' }}>
  ...
  return (
    <div className="stat">
      ...
      <div className="stat-info">
        ...
        <div className="muted" style={{ fontSize: '0.9rem', opacity: 0.7 }}>{title}</div>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div className="card table">
        <div className="table-head">
          <div className="muted">{rows.length} total items</div>
        </div>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r._id}>
                  {columns.map((c) => (
                    <td key={c.key}>
                      {c.key === 'price' ? `Rs ${String(r[c.key]).toLocaleString()}` : String(r[c.key] ?? "")}
                    </td>
                  ))}
                  <td className="td-actions" style={{ justifyContent: 'flex-end' }}>
                    {renderRight ? renderRight(r) : null}
                    {!hideEdit ? (
                      <button className="btn btn-ghost" onClick={() => onEdit?.(r)}><i className="fas fa-edit"></i> Edit</button>
                    ) : null}
                    <button className="btn btn-ghost danger" onClick={() => onDelete?.(r)}><i className="fas fa-trash"></i></button>
                  </td>
                </tr>
              ))}
              {!rows.length ? (
                <tr><td colSpan={columns.length + 1} className="muted center">No data available</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>
